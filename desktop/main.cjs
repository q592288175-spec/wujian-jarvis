...
if(!app.requestSingleInstanceLock()){app.quit()}else{
 ...
 app.whenReady().then(async()=>{
  window=new BrowserWindow({width:1500,height:980,minWidth:1100,minHeight:720,backgroundColor:'#030b15',title:'五简 JARVIS',...(process.platform==='darwin'?{titleBarStyle:'hiddenInset'}:{}),webPreferences:{preload:path.join(__dirname,'preload.cjs'),nodeIntegration:false,contextIsolation:true,sandbox:true,webSecurity:true}});
