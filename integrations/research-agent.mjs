...
async function initializeResearch(input){
 ...
 await mkdir(dir,{recursive:true});const id=randomUUID();const job={id,status:'running',stage:'正在检索官方资料并分析品种',name:contractName(q),symbol:q.symbol,created:new Date().toISOString(),dataAsOf:snapshot.receivedAt,evidence:{threeAxes:{...axesFor(q),methodVersion:axesKnowledge.version,source:axesKnowledge.source},structure:analyzeStructure(q),quote:{...brief.rows[0],preClose:q.preClose},dailyBars:(q.bars||[]).filter(b=>b.complete).slice(-25),volume:q.volume,liquidity:snapshot.liquidity,methodVersion:'1.3'}};
