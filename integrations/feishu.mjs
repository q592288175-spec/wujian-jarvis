...
async function tick(){if(busy||!ready)return;busy=true;try{
 ...
 for(const slot of dueSlots(new Date(),settings.times,calendar)){if(!await ledger.claim('schedule:'+slot.key))continue;const report=await previewFeishuReport();await mkdir(resolve(dir,'reports'),{recursive:true});await atomicWrite(resolve(dir,'reports',slot.key.replace(':','-')+'.md'),report.text);const c=config();const text=report.status==='observing'&&report.rows.length?report.text:`小木 ${slot.date} ${slot.time}：行情状态 ${report.status}，未生成正式日报，请检查采集。`;await send(c.target,text,'schedule:'+slot.key,c.type);await ledger.finish('schedule:'+slot.key,'sent');}
