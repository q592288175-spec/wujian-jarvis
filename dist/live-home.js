...
function render(){
 ...
 const label={unconfigured:'天勤账号未配置',unavailable:'行情进程未启动',stale:'行情进程已失联，显示旧快照',connecting:'天勤连接中',disconnected:'天勤连接失败',waiting:'等待行情',observing:`TqSdk 实时同步 · 已接入 ${snapshot.liquidity?.total??snapshot.quotes.length} 个主力合约`,stopped:'行情进程已停止'}[snapshot.status]||snapshot.status;
 ...
 $('.markets .panel-foot').textContent=`实时订阅 ${snapshot.liquidity?.total??snapshot.quotes.length} 个主力合约 · 研究池 ${snapshot.quotes.length} 个 · 已过滤 ${snapshot.liquidity?.excluded??0} 个（量仓均＞1万手）`;
