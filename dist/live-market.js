...
export async function mountLiveMarket(){
 ...
 host.innerHTML=`<h3>国内期货 · TqSdk 只读行情</h3><p>${esc(labels[s.status]||s.status)} · ${esc(s.source)} · 接收 ${esc(s.receivedAt)} </p><p>${esc(s.error||'涨跌幅口径：较昨结算。行情时间为交易所当地时间；休市与断线尚不自动判定。')}</p><button class="quiet" data-refresh>刷新快照</button><div style="overflow:auto"><table class="work-table"><thead><tr><th>订阅 / 实际合约</th><th>最新价</th><th>涨跌幅</th><th>成交 / 持仓</th><th>行情时间 / 交易日</th></tr></thead><tbody>${s.quotes.map(q=>`<tr><td><button data-symbol="${esc(q.symbol)}">${esc(q.requestedSymbol)}<br>${esc(q.symbol)}</button></td><td>${esc(q.last)}</td><td>${q.changePct==null?'未知':esc(q.changePct.toFixed(2))+'%'}</td><td>${esc(q.volume)} / ${esc(q.openInterest)}</td><td>${esc(q.quoteTime)}<br>交易日：${esc(q.tradingDay||'未知')}</td></tr>`).join('')}</tbody></table></div><div data-bars></div>`;
 host.querySelector('[data-refresh]').onclick=refresh;
 function draw(symbol){selected=symbol;const q=s.quotes.find(q=>q.symbol===symbol);if(!q)return;const bars=q.bars.filter(b=>[b.open,b.high,b.low,b.close].every(Number.isFinite));const area=host.querySelector('[data-bars]');if(!bars.length){area.textContent='日K尚未取得';return}const min=Math.min(...bars.map(b=>b.low)),max=Math.max(...bars.map(b=>b.high)),y=v=>150-(v-min)/(max-min||1)*140;area.innerHTML=`<h4>${esc(symbol)} · 日K（末根未完成，不用于完成K线触发）</h4><svg viewBox="0 0 720 165" role="img" aria-label="天勤实际合约日K">${bars.map((b,i)=>{const x=5+i*710/bars.length,c=b.close>=b.open?'#fa737d':'#55dfd1';return `<path stroke="${c}" d="M${x},${y(b.low)} V${y(b.high)}"/><path stroke="${c}" stroke-width="3" d="M${x},${y(b.open)} V${y(b.close)+.1}"/>`}).join('')}</svg>`}
